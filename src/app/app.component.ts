...
export class AppComponent {
  title = 'maknew';
  hrefx:any="";
  user:any={status:"",name:""};
  profile:any = false;loading:any=false;
  icon:any = environment.icon;
  redContact:any=false;
  redEmail:any=false;
  redName:any=false;
  countryCurency:any = "USD";
    ...
    this.router.events.subscribe((e) => {
      if (e instanceof NavigationEnd) {
        ...
        if(us!=null){
          ...
          if(this.user.name.length==0 || this.user.contact.length==0 || this.user.email.length==0 || this.user.cuntry==null){
            this.user.cuntry="";this.profile =true;
            if(this.user.name.length>0){this.redName=true;}
            if(this.user.contact.length>0){this.redContact=true;}
            if(this.user.email.length>0){this.redEmail=true;}
            }
            this.fetchListed();
        }
        this.hrefx = this.router.url;
      }
      if(localStorage.getItem("countrycurency")==null){this.curency();}
      else{this.countryCurency = localStorage.getItem("countrycurency");}
    });
  }

  logout(){
    localStorage.clear();
    var gsg = showAndroidToast("logout","","");
    /* if(gsg){
  ...
  curency(){
    localStorage.setItem("countrycurency",this.countryCurency);
  }
  

  updateUser:any=new FormGroup({
    cuntry: new FormControl("",[Validators.required]),
  ...
  submit(){
    if (this.updateUser.invalid) {
      this.updateUser.get('cuntry').markAsTouched();
      ...
      if(!this.loading){
        this.loading =true;
        var nuser = localStorage.getItem("user");
        if(nuser!=null){
            this.user = JSON.parse(nuser);
            
            var nwUser = this.updateUser.value;
            this.user.cuntry = this.cuntry;
            if(this.redContact){this.user.contact = nwUser.contact;}
            else{this.user.contact = this.code + nwUser.contact;}
            this.user.email = nwUser.email;
            this.user.name = nwUser.name;
            this.orgnigation.updateUser(this.user).subscribe(
              ...
              error=>{
                this.loading =false;
                if(error.status==403){
                  alert("Enter Email Id or Contact Number already used in another account.");
                }
                else{
  ...
  cuntrycode:any="";cuntry:any;code:any;
  selectCuntry(pos:any){
    var pos = pos.target.value;
    this.cuntry = this.cuntryList[pos].country_name;
    this.cuntrycode = "( "+this.cuntryList[pos].country_short_name+ ": +" +this.cuntryList[pos].country_phone_code+" )";
    this.code = "+"+this.cuntryList[pos].country_phone_code;
  }
